...
\section{Turbulence driving by smooth accretion}

Relationship between velocity dispersion of DM and virial mass:
%
\begin{align}
	\sigma_{\mathrm{DM}}^2 = (\ldots) ~ M_{200}^{1/3}
\end{align}

Temperature of an isothermal sphere made of collisionless DM particles:
%
\begin{align}
	T \propto \sigma_{\mathrm{DM}}^2,
\end{align}
%
If gas is also isothermal with same $T$, then its thermal energy $E_{\mathrm{th}}$ is
%
\begin{align}
	E_{\mathrm{th}} = (3/2) N k_\mathrm{B} T  \propto M_{200}^{5/3}
\end{align}
%
The change in the thermal energy of the gas assuming smooth accretion and a virialized cluster is then related to the mass accretion rate:
%
\begin{align}
	\frac{d \ln E_{\mathrm{th}}}{d t} = \frac{5}{3} \frac{d \ln M_{200}}{d t}
\end{align}
%
The change in the thermal energy of the gas is due to dissipative heating (and also shock heating) minus cooling. Assuming K41 and neglecting other sources of heating/cooling, we can relate the rate of change of $E_{\mathrm{th}}$ with the turbulent energy flux $\epsilon_{\mathrm{turb}}$:
%
\begin{align}
	\frac{d  E_{\mathrm{th}}}{d t} = \eta M_{200} \epsilon_{\mathrm{turb}},
\end{align}
%
($\eta$ is an order-unity coefficient) and the energy flux to the turbulent velocity amplitude $\delta u_l$ at a given scale $l$:
%
\begin{align}
	\epsilon_{\mathrm{turb}} = \left(\frac{2}{3 C}\right) \frac{\delta u_l^3}{l}
\end{align}
