...
# TODO: this probably should not be a class
class MexicanHatPowerSpectrum(SmoothingFilter):
    ...
    def compute_spectrum(self, variable, max_filter_length=None, mask=None, weight=None, optimized=False):
        ...
        k_vec = np.logspace(np.log10(self.kmin), np.log10(self.kmax), 15)/self.snap.length
        # this is a volume integral of the variance
        var_variance = np.zeros((len(k_vec),ndims))*var_unit**2*self.snap.length**3 
        ...
        for i in range(len(k_vec)):
            ...
            if (mask is None):
                var_filtered, _ = extract_turbulent_vector(snap, self, variable, filt_len,  weight, filter_type="mexican-hat", iterative=False)
                for n in range(ndims):
                    var_variance[i, n] = volume_integral(snap, var_filtered[:,n]**2, self.indicesFirstPass)
            else:
                ## equation A9 Arevalo+2012
                G_sigma1_var, _ = extract_turbulent_var(snap, self,  variable, filt_len/np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
                # note that it does not matter what unit is mask
                # because in the end it cancels out
                # but we need a unit for the following function to work
                ...
                # we use the convention that mask=1=True on the cells which we want
                # to include, and mask=0=False on the cells to exclude
                G_sigma1_mask, _ = extract_turbulent_var(snap, self,  mask*var_unit, filt_len/np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)

                G_sigma2_var, _ = extract_turbulent_var(snap, self,  variable, filt_len*np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
                G_sigma2_mask, _ = extract_turbulent_var(snap, self,  mask*var_unit, filt_len*np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
        ...
        ## this is the 3D energy spectral density
        power_spectrum3D = np.zeros((len(k_vec),ndims))*var_variance.uq*self.snap.length**3
        ...
        ## for 1D energy spectral density
        ## (we are assuming S_3D does not depend on the angular variable)
        power_spectrum1D = np.zeros((len(k_vec),ndims))*var_variance.uq*self.snap.length
