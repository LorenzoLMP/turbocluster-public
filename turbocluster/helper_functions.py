...
def extract_turbulent_var(snap, sf, variable, filter_length, weight, 
                             filter_type="gaussian", 
                             iterative=False, selection=None):

    filt_var = sf.filter_variable(variable, filter_length, weight=weight, 
                                      filter_type=filter_type, iterative=iterative, 
                                  selection=selection)
    ...
    turbVar[~sf.indicesFirstPass*sf.index] = 0 ## this is for the thin buffer, which we set to zero
