...
@cuda.jit()
def apply_derivative(oldIndex, pos, hsml, tile_index, 
                        ...
                        if check_distance(ip_tile_x, ip_tile_y, ip_tile_z,
                            ...
                            for ip_other in range(start_index, start_index + n_particles):
                                ...
                                if dist < filter_window *  filter_length:
                                    # gradient of kernel
                                    grad_x, grad_y, grad_z = gradient_gaussian((xp, yp, zp), pos[ip_other], 
                                                                   dist, filter_length)
                                    # gaussian_kernel(dist, filter_length) 
                                    # weight_tmp *= weights[ip_other]
                                    # volume of voronoi cell
                                    volume = (4./3.)*cp.pi*hsml[ip_other]**3
                                    # normalization of the integral
                                    # normalization += weight_tmp
                                    grad_x_VarRegister += variable[ip_other] * grad_x * volume
                                    grad_y_VarRegister += variable[ip_other] * grad_y * volume
                                    grad_z_VarRegister += variable[ip_other] * grad_z * volume
...
@cuda.jit()
def apply_derivative_vector(oldIndex, pos, hsml, tile_index, 
                            start_index_for_tile, particles_per_tile, tile_widths,
                            variable_x, variable_y, variable_z, 
                            offsets, npixs, center, widths, filter_lengths, 
                            grad_x_vec_x, grad_x_vec_y, grad_x_vec_z, 
                            grad_y_vec_x, grad_y_vec_y, grad_y_vec_z,
                            grad_z_vec_x, grad_z_vec_y, grad_z_vec_z,
                        ...
                        if check_distance(ip_tile_x, ip_tile_y, ip_tile_z,
                            ...
                            for ip_other in range(start_index, start_index + n_particles):
                                ...
                                if dist < filter_window *  filter_length:
                                    ...
                                    grad_x_Vec_x_Reg += variable_x[ip_other] * grad_x * volume
                                    grad_y_Vec_x_Reg += variable_x[ip_other] * grad_y * volume
                                    grad_z_Vec_x_Reg += variable_x[ip_other] * grad_z * volume

                                    grad_x_Vec_y_Reg += variable_y[ip_other] * grad_x * volume
                                    grad_y_Vec_y_Reg += variable_y[ip_other] * grad_y * volume
                                    grad_z_Vec_y_Reg += variable_y[ip_other] * grad_z * volume
                                    
                                    grad_x_Vec_z_Reg += variable_z[ip_other] * grad_x * volume
                                    grad_y_Vec_z_Reg += variable_z[ip_other] * grad_y * volume
                                    grad_z_Vec_z_Reg += variable_z[ip_other] * grad_z * volume
