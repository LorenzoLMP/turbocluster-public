...
class SmoothingFilter:
    ...
    #         apply_filter[self.blocks_1d, self.threadsperblock](pos, hsml, tile_index, start_index_for_tile,particles_per_tile, tile_widths,variable, weights, offsets, npixs, center, widths,filter_lengths, smooth_var, filter_type, hitsNeighbours,   isParticleInDomain, iterativeFilter, hasConverged, numIterations, filter_lengths_out, self.multiplier, max_search_radius, isParticleInSelection)
    #         nvtx.end_range(rng)
    #     elif self.spherical:
    #         rng = nvtx.start_range(message="spherical filter")
    #         apply_filter_spherical[self.blocks_1d, self.threadsperblock](pos, hsml, tile_index, start_index_for_tile,particles_per_tile, spacings,variable, weights, nSects, center, rMin, rMax, _rMin, _rMax,filter_lengths, smooth_var, filter_type, hitsNeighbours,isParticleInDomain, typeGrid, power, iterativeFilter,hasConverged, numIterations, filter_lengths_out, self.multiplier, max_search_radius, isParticleInSelection)
    #         nvtx.end_range(rng)
    ...
    def _apply_filter_gpu_optimized(self, variable_str, weight, filter_type, iterative):
        ...
        (optimized=False)
        """
        # TODO: need to check this one in fact
        if self.spherical:
