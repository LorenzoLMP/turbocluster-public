...
def accumulate_quantity_per_tile(tile_index, quantity_per_tile, quantity):
    ...
    if ip < tile_index.shape[0]:
        ip_tile_x, ip_tile_y, ip_tile_z = tile_index[ip, :]
        cuda.atomic.add(
            quantity_per_tile, (ip_tile_x, ip_tile_y, ip_tile_z), quantity[ip]
        )
...
@cuda.jit
def compactify_kernel(
    occupancy_arr,
    cumulative_occupancy_flat,
    npixs,
    Nmax,
    particles_per_tile,
    start_index_for_tile,
    numBlocksCompactGrid,
    compactGrid,
):
    """ """
    ip = cuda.grid(1)
    ...
    if ip < numBlocksFullGrid:
        ...
        for i in range(numBlocksNeeded):
            ...
            compactGrid[newPos - 1 - i, 1] = (
                start_index_for_tile[ip] + (numBlocksNeeded - 1 - i) * Nmax
            )
            if i > 0:
                compactGrid[newPos - 1 - i, 2] = Nmax
            else:
                compactGrid[newPos - 1 - i, 2] = (
                    particles_per_tile[ip] - (numBlocksNeeded - 1) * Nmax
                )
...
class CartesianTiling:
    ...
    def sort_particles_in_tiles(self):
        ...
        self.tile_index = cp.zeros((self.Np, 3), dtype=int)
        ...
        get_tile_information[self.blocks_1d, self.threadsperblock](
            self.tile_index, self.particles_per_tile, self.start_index_for_tile
        )
        nvtx.end_range(rng)
    ...
    def compactify_grid(self, Nmax):
        ...
        compactify_kernel[blocks_1d, threadsperblock](
            occupancy_arr,
            cumulative_occupancy,
            self.npixs,
            Nmax,
            self.particles_per_tile.flatten(),
            self.start_index_for_tile.flatten(),
            numBlocksCompactGrid,
            compactGrid,
        )
