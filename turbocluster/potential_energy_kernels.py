...
def grav_pot_kernel(r, h):
    ...
    u = r/h
    if (u < 0.5):
        phi *= (14./5.)*u - (16./3.)*u**3 + (48./5.)*u**5 - (32./5.)*u**6
    elif (u >= 0.5 and u < 1):
...
def compute_potential_energy_coarse(pos, mass, smoothing_length,
    ...
    if (ip < numBlocks):
        ...
        for ip_other in range(0,numBlocks):
            ...
            max_hsml_other_tile = max_hsml_per_tile[ip_other_tile_x, ip_other_tile_y, ip_other_tile_z]
                    ...
                    if distance_tile_other_tile < H/(2*tan_theta0) or max_hsml_tiles > abs(distance_tile_other_tile - H):
                        ...
                        for ip_other in range(start_index_other_tile,
                            ...
                            if (ip != ip_other):
                                ...
                                pot_tmp = - mass[ip] * mass[ip_other] * grav_pot_kernel(r, hsml)
                            else:
                                # pass
                                ## Eq.108 of 10.1111/j.1365-2966.2009.15715.x
                                ## limit for r->0
                                pot_tmp = - mass[ip]*mass[ip_other]*(14./5.)/hsml

                            cuda.atomic.add(potential_in_tile, (ip_tile_x, ip_tile_y, ip_tile_z), pot_tmp )
                            particles_hit[ip] += 1
