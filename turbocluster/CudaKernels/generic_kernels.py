...
def sphere_kernel(dist, filter_length):
    ...
    weight = 1.0 / (4.0 * cp.pi * filter_length**3 / 3.0)
...
def gaussian_kernel(dist, filter_length):
    ...
    # weight = math.exp(-0.5*(dist/filter_length)**2)
    weight = (
        math.exp(-0.5 * (dist / filter_length) ** 2)
        / filter_length**3
        / (2.0 * cp.pi) ** (3.0 / 2.0)
    )
...
def mexican_kernel_2(dist, filter_length):
    ...
    weight_1 = gaussian_kernel(dist, filter_length / (1.0 + epsilon) ** 0.5)
    weight_2 = gaussian_kernel(dist, filter_length * (1.0 + epsilon) ** 0.5)
...
@cuda.jit()
def compactify_particles(
    pos, tile_index, cumulative_occupancy_flat, isParticleInDomain, oldIndex
):
    """ """
    ip = cuda.grid(1)
    numParticles = pos.shape[0]
