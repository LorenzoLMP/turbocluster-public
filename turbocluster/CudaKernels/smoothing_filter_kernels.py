    ...
    if ip < oldIndex.shape[0]:
        ...
        while filter_length <= max_filter_length and not hasIterationConverged:
                    ...
                    for tile_y in range(ip_tile_y_min, ip_tile_y_max + 1):
                        for tile_z in range(ip_tile_z_min, ip_tile_z_max + 1):
                            if check_distance(
                                ...
                                start_index = start_index_for_tile[
                                ...
                                for ip_other in range(
                                    ...
                                    if dist < filter_window * filter_length:
                                        ...
                                        weight_tmp *= (
                                            (4.0 / 3.0) * cp.pi * hsml[ip_other] ** 3
                                        )
                                        # normalization of the integral
                                        normalization += weight_tmp
                                        smoothVarRegister += (
                                            variable[ip_other] * weight_tmp
                                        )
                                ...
                                for ip_other in range(
                                    ...
                                    if dist < filter_window * filter_length:
                                        ...
                                        normalization += (
                                            ...
                                            * hsml[ip_other] ** 3
                                            * sphere_kernel(
                                                dist, filter_window * filter_length
                                            )
                                        )
                                        smoothVarRegister += (
                                            variable[ip_other] * weight_tmp
            ...
            if iterativeFilter == 0:  # no iterative
                ...
                if (numIter > 1) and (
                    increase <= toleranceParam * (0.0 + math.fabs(turbFieldOld))
                ):
...
@cuda.jit()
def apply_filter_optimized_vector(
    oldIndex,
    pos,
    hsml,
    tile_index,
    ...
    if ip < oldIndex.shape[0]:
        ...
        while filter_length <= max_filter_length and not hasIterationConverged:
            ...
            smoothVarRegister_x, smoothVarRegister_y, smoothVarRegister_z = (
                0.0,
                0.0,
                0.0,
            )
                            ...
                            if check_distance(
                                ...
                                for ip_other in range(
                                    ...
                                    if dist < filter_window * filter_length:
                                        ...
                                        smoothVarRegister_x += (
                                            variable_x[ip_other] * weight_tmp
                                        )
                                        smoothVarRegister_y += (
                                            variable_y[ip_other] * weight_tmp
                                        )
                                        smoothVarRegister_z += (
                                            variable_z[ip_other] * weight_tmp
                                        )
                                        numInteractingPartNew += 1
            ...
            if iterativeFilter == 0:  # no iterative
                ...
                norm_Old = math.sqrt(
                    turbFieldOld_x**2 + turbFieldOld_y**2 + turbFieldOld_z**2
                )
                # relIncrease =  math.sqrt((turbFieldNew_x - turbFieldOld_x)**2 + \
                # + (turbFieldNew_y - turbFieldOld_y)**2 + \
                # (turbFieldNew_z - turbFieldOld_z)**2 ) / norm_Old
                increase = math.sqrt(
                    (turbFieldNew_x - turbFieldOld_x) ** 2
                    + (turbFieldNew_y - turbFieldOld_y) ** 2
                ...
                if (numIter > 1) and (
                    increase <= toleranceParam * (0.0 + math.fabs(norm_Old))
                ):
                    if numInteractingPartNew > numInteractingPartOld:
                        if numInteractingPartNew > 8:
                            hasIterationConverged = True
...
def compute_max_hsml(hsml, compactGrid, maxHsmlPerBlock):
    ...
    if ip < numBlocksCompGrid:
        ...
        for i in range(compactGrid[ip, 2]):
            ...
            hsmlMaxTmp = (
                hsml[particleIp] if (hsml[particleIp] > hsmlMaxTmp) else hsmlMaxTmp
            )
...
@cuda.jit()
def compactify_in_domain(
    fullCompactGrid, cumulative_occupancy, isBlockInDomain, compactGridInDomain
):
    """ """
    ip = cuda.grid(1)
