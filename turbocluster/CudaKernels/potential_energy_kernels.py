...
@cuda.jit()
def compute_potential_energy_coarse(pos, mass, smoothing_length,
    ...
    if (ip < numBlocks):
        ...
        start_index_this_tile = start_index_for_tile[ip_tile_x,
                                                     ip_tile_y, ip_tile_z]
        num_particles_this_tile = particles_per_tile[ip_tile_x,
                                                     ip_tile_y, ip_tile_z]
        ...
        H = math.sqrt(tile_widths[0]**2
                      + tile_widths[1]**2 + tile_widths[2]**2)
        ...
        for ip_other in range(0, numBlocks):
            ...
            other_tile_coord_x = offsets[0] + \
                (ip_other_tile_x + 0.5) * tile_widths[0]
            other_tile_coord_y = offsets[1] + \
                (ip_other_tile_y + 0.5) * tile_widths[1]
            other_tile_coord_z = offsets[2] + \
                (ip_other_tile_z + 0.5) * tile_widths[2]

            start_index_other_tile = start_index_for_tile[ip_other_tile_x,
                                                          ip_other_tile_y, ip_other_tile_z]
            num_particles_other_tile = particles_per_tile[ip_other_tile_x,
                                                          ip_other_tile_y, ip_other_tile_z]
            mass_other_tile = mass_per_tile[ip_other_tile_x,
                                            ip_other_tile_y, ip_other_tile_z]
            max_hsml_other_tile = max_hsml_per_tile[ip_other_tile_x,
                                                    ip_other_tile_y, ip_other_tile_z]
            ...
            if distance_ip_ip_other < H / (2 * tan_theta0) or max_hsml_tiles > abs(distance_ip_ip_other - H):
                ...
                potential_in_tile[ip_tile_x, ip_tile_y, ip_tile_z] -= mass_this_tile * \
                    mass_other_tile / distance_ip_ip_other
                tiles_hit[ip_tile_x, ip_tile_y, ip_tile_z] += 1


@cuda.jit()
def compute_potential_energy_N2(pos, mass, smoothing_length,
                                tile_index, start_index_for_tile,
                                particles_per_tile, mass_per_tile,
                                max_hsml_per_tile,
                                tile_widths, tan_theta0,
    ...
    if (ip < numParticles):
        ...
        ip_tile_x_min = ip_tile_x - (
            dist_max + tile_widths[0] / 2) // tile_widths[0]
        ip_tile_x_max = ip_tile_x + (
            dist_max + tile_widths[0] / 2) // tile_widths[0]

        ip_tile_y_min = ip_tile_y - (
            dist_max + tile_widths[1] / 2) // tile_widths[1]
        ip_tile_y_max = ip_tile_y + (
            dist_max + tile_widths[1] / 2) // tile_widths[1]

        ip_tile_z_min = ip_tile_z - (
            dist_max + tile_widths[2] / 2) // tile_widths[2]
        ...
        for ip_other_tile_x in range(ip_tile_x_min, ip_tile_x_max + 1):
            for ip_other_tile_y in range(ip_tile_y_min, ip_tile_y_max + 1):
                for ip_other_tile_z in range(ip_tile_z_min, ip_tile_z_max + 1):
                    ...
                    distance_tile_other_tile = distance((tile_coord_x, tile_coord_y, tile_coord_z),
                                                        (other_tile_coord_x, other_tile_coord_y, other_tile_coord_z))
                    ...
                    max_hsml_tiles = max(
                        max_hsml_this_tile, max_hsml_other_tile)

                    if distance_tile_other_tile < H / (2 * tan_theta0) or max_hsml_tiles > abs(distance_tile_other_tile - H):
                        # includes the case when ip == ip_other
                        # do the particle-particle loop
                        # the second condition refers to case when hsml is potentially
                        # very large and partially overlaps with other tile
                        # ip, ip_other here refer to particle indices
                        for ip_other in range(start_index_other_tile,
                                              start_index_other_tile + num_particles_other_tile):
                            # for symmetry use the max hsml of the two particles in
                            # calculation of potential
                            hsml = max(
                                smoothing_length[ip], smoothing_length[ip_other])
                            # exclude self-interaction
                            if (ip != ip_other):
                                # pass
                                r = distance(pos[ip], pos[ip_other])
                                pot_tmp = - \
                                    mass[ip] * mass[ip_other] * \
                                ...
                                # limit for r->0
                                pot_tmp = - mass[ip] * \
                                    mass[ip_other] * (14. / 5.) / hsml

                            cuda.atomic.add(
                                potential_in_tile, (ip_tile_x, ip_tile_y, ip_tile_z), pot_tmp)
                            particles_hit[ip] += 1
