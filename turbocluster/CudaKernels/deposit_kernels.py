...
def weight_psc(xg, xi, h, Deltax):
    ...
    if (t <= 0.0):
        ...
        if (t <= Deltax):
            ...
            weight = Deltax / (6.*h**3) * ( Deltax**2 - 3.*Deltax*t + 3*t**2)
    elif (t > h and t <= 2.*h):
        if (t <= Deltax):
            weight = ( 3.*h**3 - 9.*h**2*t + 9.*h*t**2 - 2.*t**3) / (6.*h**3) 
        elif (t > Deltax and t <= Deltax + h):
            weight = (h - t) * (2.*h**2 - 7.*h*t + 2.*t**2) / (6.*h**3)
            weight += (h**3 - (t - Deltax)**3 ) / (6.*h**3)
        else:
            weight = Deltax * (-2.*Deltax**2 - 9.*Deltax*h - 9.*h**2 + 6.*(Deltax + 3.*h)*t - 6.*t**2)
            weight /= (6.*h**3)

    elif (t > 2.*h and t <= 3.*h):
        if (t <= Deltax):
            weight = ( t**3 - 9.*h*t**2 + 27.*h**2*t - 21.*h**3 ) / (6.*h**3)
        else:
            weight = Deltax**3 - 21.*h**3 - 3.*(Deltax**2 - 9.*h**2)*t + 3.*(Deltax - 3.*h)*t**2
            weight /= (6.*h**3) 
