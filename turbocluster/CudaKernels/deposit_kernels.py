...
def weight_tsc(xg, xi, h, Deltax):
    ...
    if (2. * h < Deltax):
        ...
        if (t < 0.0):
            ...
            if (t <= h):
                weight = (1. / h) * Deltax * (t - Deltax / 2.0)
            elif (t > h):
                weight = (1. / h) * (Deltax * (t - Deltax / 2.0) - (t - h)**2)
...
def weight_psc(xg, xi, h, Deltax):
    ...
    """
    # t is the overlap
    t = Deltax / 2.0 + 3. * h / 2. - math.fabs(xg - xi)
    weight = 0.0
    ...
    if (t <= 0.0):
        ...
        if (t <= Deltax):
            ...
            weight = Deltax / (6. * h**3) * (Deltax**2
                                             - 3. * Deltax * t + 3 * t**2)
    elif (t > h and t <= 2. * h):
        if (t <= Deltax):
            weight = (3. * h**3 - 9. * h**2 * t + 9.
                      * h * t**2 - 2. * t**3) / (6. * h**3)
        elif (t > Deltax and t <= Deltax + h):
            weight = (h - t) * (2. * h**2 - 7.
                                * h * t + 2. * t**2) / (6. * h**3)
            weight += (h**3 - (t - Deltax)**3) / (6. * h**3)
        else:
            weight = Deltax * (-2. * Deltax**2 - 9. * Deltax * h
                               - 9. * h**2 + 6. * (Deltax + 3. * h) * t - 6. * t**2)
            weight /= (6. * h**3)

    elif (t > 2. * h and t <= 3. * h):
        if (t <= Deltax):
            weight = (t**3 - 9. * h * t**2 + 27. * h
                      ** 2 * t - 21. * h**3) / (6. * h**3)
        else:
            weight = Deltax**3 - 21. * h**3 - 3. * \
                (Deltax**2 - 9. * h**2) * t + 3. * (Deltax - 3. * h) * t**2
            weight /= (6. * h**3)
            ...
            if (xgrid_idx >= 0) and (xgrid_idx < nx + 1):
                if (ygrid_idx >= 0) and (ygrid_idx < ny + 1):
                    if (zgrid_idx >= 0) and (zgrid_idx < nz + 1):
                        ...
                        cuda.atomic.add(
                            scratch, (xgrid_idx, ygrid_idx, zgrid_idx), 1.0)
            ...
            for tile_x in range(ip_tile_x_min, ip_tile_x_max + 2):
                for tile_y in range(ip_tile_y_min, ip_tile_y_max + 2):
                    for tile_z in range(ip_tile_z_min, ip_tile_z_max + 2):
                        ...
                        xcoord = center[0] - widths[0] / \
                            2 + tile_x * tile_widths[0]
                        ycoord = center[1] - widths[1] / \
                            2 + tile_y * tile_widths[1]
                        zcoord = center[2] - widths[2] / \
                            2 + tile_z * tile_widths[2]
                        ...
                        if (xgrid_idx >= 0) and (xgrid_idx < nx + 1):
                            if (ygrid_idx >= 0) and (ygrid_idx < ny + 1):
                                if (zgrid_idx >= 0) and (zgrid_idx < nz + 1):
                                    ...
                                    weight_tmp = weight_cic(xcoord, xp, hsml[ip], tile_widths[0]) * \
                                        weight_cic(ycoord, yp, hsml[ip], tile_widths[1]) * \
                                        weight_cic(zcoord, zp, hsml[ip], tile_widths[2]) * \
                                        weightVar / voxel
                        ...
                        if (xgrid_idx >= 0) and (xgrid_idx < nx + 1):
                            if (ygrid_idx >= 0) and (ygrid_idx < ny + 1):
                                if (zgrid_idx >= 0) and (zgrid_idx < nz + 1):
                                    ...
                                    weight_tmp = weight_tsc(xcoord, xp, hsml[ip], tile_widths[0]) * \
                                        weight_tsc(ycoord, yp, hsml[ip], tile_widths[1]) * \
                                        weight_tsc(zcoord, zp, hsml[ip], tile_widths[2]) * \
                                        weightVar / voxel
                                    # weight_tmp = 1.0
                                    # the following two need to be atomic add
                                    # cuda.atomic.add(scratch, (xgrid_idx, ygrid_idx, zgrid_idx),
