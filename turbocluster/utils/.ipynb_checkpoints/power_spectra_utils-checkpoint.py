...
def power_spectrum1d(depo, deposited_variable, **kwargs):
        ...
        if pa.settings.use_units:
            k1d = k1d / L_unit
            powerspectr = powerspectr * L_unit**4 * variable_unit**2 # (for energy/frequency: 3 powers for Lx*Ly*Lz + 1 power for 1/deltak)      
...
def mexicanhatPowerSpectrum(snap, sf, variable, max_filter_length=None, mask=None, weight=None, optimized=False):
    ...
    for i in range(len(k_vec)):
        ...
        if (mask is None):
            var_filtered, _ = extract_turbulent_vector(snap, sf, variable, filt_len,  weight, filter_type="mexican-hat", iterative=False)
            for n in range(ndims):
                var_variance[i, n] = volume_integral(snap, var_filtered[:,n]**2, sf.indicesFirstPass)
        else:
            ## equation A9 Arevalo+2012
            G_sigma1_var, _ = extract_turbulent_var(snap, sf,  variable, filt_len/np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
            ...
            G_sigma1_mask, _ = extract_turbulent_var(snap, sf,  mask*var_unit, filt_len/np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)

            G_sigma2_var, _ = extract_turbulent_var(snap, sf,  variable, filt_len*np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
            G_sigma2_mask, _ = extract_turbulent_var(snap, sf,  mask*var_unit, filt_len*np.sqrt(1.0+epsilon),  weight, filter_type="gaussian", iterative=False)
            ...
            for n in range(ndims):
                ...
                vol_tot = volume_integral(snap, np.ones(mask.shape), sf.indicesFirstPass)

                vol_non_masked = volume_integral(snap, mask, sf.indicesFirstPass)

                # equation A10 of Arevalo+2012
                var_variance[i, n] = (vol_tot/vol_non_masked) * volume_integral(snap, S_k**2, sf.indicesFirstPass)
            
    # equation A11 of Arevalo+2012 replacing k_r -> k / (2 \pi) 

    ## this is the 3D energy spectral density
    power_spectrum3D = np.zeros((len(k_vec),ndims))*var_variance.uq*sf.snap.length**3

    for n in range(ndims):
        power_spectrum3D[:,n] = (var_variance[:, n]/k_vec**3)*(np.pi**(3./2.)*2**(11./2.))/(3.*5./2.) 
    ...
    for n in range(ndims):
        power_spectrum1D[:,n] = power_spectrum3D[n] * 4.0*np.pi*(k_vec/(2.*np.pi))**2 
