...
from ..CudaKernels.power_spectra_kernels import nd_window, gpu_power_spectrum1d
from ..CudaKernels.generic_kernels import epsilon
from ..helper_functions import volume_integral, extract_turbulent_var
...
def power_spectrum1d(depo, deposited_variable, **kwargs):
    ...
    if "window" in kwargs:
        depo_variable, ndim_window = nd_window(depo_variable, kwargs["window"])
    ...
    blocks_1d = (Ntotalcomplex + (depo.threadsperblock - 1)) // depo.threadsperblock

    # launch kernel
    gpu_power_spectrum1d[blocks_1d, depo.threadsperblock](
        hat_depo_variable, d_wavenum, (Nx, Ny, Nz), d_powerspectr
    )

    powerspectr = (
        cp.asnumpy(d_powerspectr) * (Lx * Ly * Lz) * (2.0 * np.pi / deltak)
    ...
    if pa.settings.use_units:
        k1d = k1d / L_unit
        powerspectr = (
            powerspectr * L_unit**4 * variable_unit**2
...
def mexicanhatPowerSpectrum(
    snap, sf, variable, max_filter_length=None, mask=None, weight=None, optimized=False
):
    ...
    if max_filter_length is None:
        max_filter_length = np.max(sf.widths) / 7.0
    if max_filter_length > sf.max_search_radius / sf.multiplier:
        raise RuntimeError(
            "The chosen filter length (x4) is larger than the \
    ...
    for i in range(len(k_vec)):
        ...
        if mask is None:
            var_filtered, _ = extract_turbulent_var(
                snap,
                sf,
                variable,
                filt_len,
            ...
            G_sigma1_var, _ = extract_turbulent_var(
                snap,
                sf,
                variable,
                filt_len / np.sqrt(1.0 + epsilon),
                weight,
            ...
            G_sigma1_mask, _ = extract_turbulent_var(
                snap,
                sf,
                mask * var_unit,
                filt_len / np.sqrt(1.0 + epsilon),
            ...
            G_sigma2_var, _ = extract_turbulent_var(
                snap,
                sf,
                variable,
                filt_len * np.sqrt(1.0 + epsilon),
            ...
            G_sigma2_mask, _ = extract_turbulent_var(
                snap,
                sf,
                mask * var_unit,
                filt_len * np.sqrt(1.0 + epsilon),
            ...
            for n in range(ndims):
                ...
                S_k = (
                    (
                        G_sigma1_var[:, n] / G_sigma1_mask
                        - G_sigma2_var[:, n] / G_sigma2_mask
                    )
                    * mask
                    * var_unit
                )
                ...
                vol_tot = volume_integral(
                    snap, np.ones(mask.shape), sf.indicesFirstPass
                )

                vol_non_masked = volume_integral(snap, mask, sf.indicesFirstPass)

                # equation A10 of Arevalo+2012
                var_variance[i, n] = (vol_tot / vol_non_masked) * volume_integral(
                    snap, S_k**2, sf.indicesFirstPass
                )
    ...
    power_spectrum3D = (
        np.zeros((len(k_vec), ndims)) * var_variance.uq * sf.snap.length**3
    )

    for n in range(ndims):
        power_spectrum3D[:, n] = (
            (var_variance[:, n] / k_vec**3)
            * (np.pi ** (3.0 / 2.0) * 2 ** (11.0 / 2.0))
            / (3.0 * 5.0 / 2.0)
        )

    # for 1D energy spectral density
    # (we are assuming S_3D does not depend on the angular variable)
    power_spectrum1D = np.zeros((len(k_vec), ndims)) * var_variance.uq * sf.snap.length

    for n in range(ndims):
        power_spectrum1D[:, n] = (
            power_spectrum3D[n] * 4.0 * np.pi * (k_vec / (2.0 * np.pi)) ** 2
        )
