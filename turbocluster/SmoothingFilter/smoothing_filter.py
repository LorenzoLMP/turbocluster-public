...
from ..CudaKernels.smoothing_filter_kernels import (
    apply_filter_optimized,
    apply_filter_optimized_vector,
)
from ..CudaKernels.derivative_smooth_filt_kernels import (
    apply_derivative,
    apply_derivative_vector,
)
from ..CudaKernels.generic_kernels import check_particle, compactify_particles
...
class SmoothingFilter(DataGpuInit):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pa.settings.use_units:
            # use units
            assert (
                search_radius.unit == self.code_length.unit
        ...
        self.extra_layer_thickness = (
            np.max(self.hsml[self.index]) + self.max_search_radius
        )
        if pa.settings.use_units:
        ...
        # Create tiling
        if self.cartesian:
            self.tile = CartesianTiling(
                self.gpu_variables["pos"],
                self.gpu_variables["center"],
                self.gpu_variables["widths"],
                self.extra_layer_thickness,
                npix=self.npix,
    ...
    def filter_variable(
        ...
        if isinstance(filter_length.value, np.ndarray):
            ...
            self.gpu_variables["filter_lengths"] = (
                cp.ones(self.Np) * filter_length.value
            )
    ...
    def _apply_filter_gpu_optimized(self, variable_str, weight, filter_type, iterative):
        ...
        if cp.max(filter_lengths) > self.extra_layer_thickness:
            err_msg = (
                f"{cp.max(filter_lengths)} is larger than {self.extra_layer_thickness}"
            )
        ...
        compactify_particles[self.blocks_1d, self.threadsperblock](
            pos,
            tile_index,
            cumulative_occupancy.flatten(),
            isParticleInDomain,
            oldIndex,
        )
        ...
        blocks_1d = (
            numParticlesInDomain + (self.threadsperblock - 1)
        ...
        if len(variable.shape) > 1:
            ...
            apply_filter_optimized_vector[blocks_1d, self.threadsperblock](
                oldIndex,
                pos,
                hsml,
                tile_index,
            ...
            # is a scalar
            apply_filter_optimized[blocks_1d, self.threadsperblock](
                oldIndex,
                pos,
                hsml,
                tile_index,
        ...
        if iterative:
            ...
            print(
                "%.2f percent of particles (%d / %d) has converged"
    ...
    def _apply_derivative_gpu(self, variable_str, filter_type):
        ...
        if len(variable.shape) > 1:
            # is a vector
            apply_derivative_vector[blocks_1d, self.threadsperblock](
                oldIndex,
                pos,
                hsml,
                tile_index,
            ...
            # is a scalar
            apply_derivative[blocks_1d, self.threadsperblock](
                oldIndex,
                pos,
                hsml,
                tile_index,
